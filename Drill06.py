...
def handle_events():
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            points.append((event.x,  TUK_HEIGHT - 1 - event.y)) # 클릭한 위치 새로운 점 추가
...
def set_new_target_arrow():
    ...
    if points:  #points 리스트 안에 남아있는 점이 있으면.
        ...
        action = 3 if action == 1 else 2 # 이전에 소년이 오른쪽으로 이동중이면, 소년이 idle 동작시 우측을 바라보도록
        frame = 0
        target_exists = False
def render_world():
    clear_canvas()
...
def update_world():
    ...
    if target_exists:
        if t <= 1.0:
            ...
            del points[0]
            set_new_target_arrow()
    elif points:  # 타겟이 없는 한, 새로운 포인트가 생기면
        set_new_target_arrow()
